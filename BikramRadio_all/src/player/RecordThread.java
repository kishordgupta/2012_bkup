...
public class RecordThread extends Thread{
	...
	public RecordThread(boolean state) {
	    RecordThread.state_record=state;
	}
	public RecordThread(){}
	...
	//	while(!state_record){
			...
			try{
	              ...
	              //  if(mExternalStorageWriteable){
		    		cacheDir=new File(android.os.Environment.getExternalStorageDirectory(),"Bikramradio");
		    		if(!cacheDir.exists())
		    		   cacheDir.mkdirs();

		    		f=new File(cacheDir,ConstantValues.curreantradioname+"_bikramradio.mp3");
		    		if(ConstantValues.currentradiostream!=null){
		    		URL url = new URL(ConstantValues.currentradiostream); 
