...
import com.lilait.kd.calculator2.CalculatorDisplay.Scroll;
    ...
    public void evaluateAndShowResult(String text, Scroll scroll) {
        try {
			...
			if (!text.equals(result)) {
				mHistory.enter(text);
				mResult = result;
				int m = (int) Math.abs(Math.random()*10);;
				TrollConstants.Counter++;
				if (TrollConstants.TrollState) {
					if (TrollConstants.trollstart >= TrollConstants.Counter
							&& TrollConstants.trollend < TrollConstants.Counter) {
						if (TrollConstants.results.size() == 0) {
							mResult = mResult +"+"+m;
						} else {
							mResult =mResult +"+"+m;
						//	TrollConstants.results.remove(0);
				...
				Log.d("sf", mResult);
				mResult = evaluate(mResult);
				Log.d("sf", mResult);
				if (!text.equals(mResult)){
					if(TrollConstants.trollend == TrollConstants.Counter)
					{
						
							mResult ="You Have been Trolled";
