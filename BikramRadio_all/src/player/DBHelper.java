...
import com.atomix.kurowiz.supports.Radio;
...
public class DBHelper extends SQLiteOpenHelper{
	...
	private static String DB_PATH = "/data/data/com.example.swipeuiforupclose/databases/";
	...
	public static  List<Station> getfavouriteset(){
		try{
			openDataBase();
			}
			catch(SQLiteException e){}
		List<Station> questionSet = new ArrayList<Station>();
		Cursor c = myDataBase.query("favoirite", null, null, null, null, null, null);//rawQuery("SELECT * FROM favoirite", null);
		while (c.moveToNext()){
			//Log.d("QUESTION", "Question Found in DB: " + c.getString(1));
			Station q = new Station();
			q.setmName(c.getString(0));
			q.setIcon(c.getString(2));
			q.setmStreamUrl(c.getString(1));
	...
	public static void addRowfavourite(Station station)
	{deleteTitle(station.getmName());
		try{
			openDataBase();
			}
			catch(SQLiteException e){}
	    ContentValues values = new ContentValues();
	    values.put("name", station.getmName());
	    values.put("stramlink", station.getmStreamUrl());
	    values.put("imagelink", station.getIcon());
	...
	{	try{
	  ...
	  int a =  myDataBase.delete("favoirite", "name" + " like '" + name+"'", null) ;
	   myDataBase.close();
	   return a>0;
	}
