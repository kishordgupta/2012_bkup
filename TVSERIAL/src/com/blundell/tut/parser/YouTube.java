...
public class YouTube {
	...
	public ArrayList<Video> getVideos(String username) throws YouTubeException {
		...
		try {
			...
			JSONArray results = resultSet.getJSONObject("data").getJSONArray(
					"items");
	...
	//	String url = feedUrl(username);
		HttpGet request = new HttpGet(username);
		/*request.addHeader("User-Agent",
				"Sunlight's Congress Android App (http://github.com/sunlightlabs/congress");
*/
	...
	private String feedUrl(String username) {
		return "https://gdata.youtube.com/feeds/api/videos?v=2&alt=jsonc&q="
				+ username + "&max-results=10&orderby=relevance";
	}
}