...
public class DBHelper extends SQLiteOpenHelper{
	...
	private static String DB_PATH = "/data/data/com.siliconorchard.ephesustravelapp/databases/";
	...
	public static  List<CityName> getfavouriteset(String learnOnEphesus){
		try{
			openDataBase();
			}
			catch(SQLiteException e){}
		List<CityName> questionSet = new ArrayList<CityName>();
		Cursor c = myDataBase.rawQuery("SELECT  title  FROM a where category like '"+learnOnEphesus+"'", null);
		while (c.moveToNext()){
	...
	public static List<CityInfo> geteventinfoistSet() {
		...
		Cursor c = myDataBase.rawQuery("SELECT * FROM a where title like '"+ConstantValues.currentcityname+"'", null);
		while (c.moveToNext()){
			//Log.d("QUESTION", "Question Found in DB: " + c.getString(1));
			CityInfo q = new CityInfo();
			q.City_name=c.getString(1);
			q.city_favourite=c.getString(2);
			q.city_description=c.getString(3);
			q.City_history=c.getString(4);
		    
		    ConstantValues.curreantevent=q;
			questionSet.add(q);
		}
		c.close();
		myDataBase.close();
